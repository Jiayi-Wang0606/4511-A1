...
class BERTopicWrapper(TopicModel):
    ...
    def prepare_topic_data(
        ...
        if self.model.c_tf_idf_ is None:
            ...
            topic_labels = self.model.topics_
        document_topic_matrix = label_binarize(topic_labels, classes=list(self.model.topic_sizes_.keys()))
        ...
        if self.model.topic_labels_:
            topic_names = [
                self.model.topic_labels_[topic] for topic in list(self.model.topic_sizes_.keys())
            ]
        else:
        ...
        def transform(corpus: list[str]):
            ...
            return label_binarize(topic_labels, classes=list(self.model.topic_sizes_.keys()))
